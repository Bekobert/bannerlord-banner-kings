...
namespace BannerKings.Behaviours
{
    public class BKAIVisitSettlementBehavior : BannerKingsBehavior
    {
        ...
        private void AiHourlyTick(MobileParty mobileParty, PartyThinkParams p)
        {
                ...
                !mobileParty.MapFaction.isKingdomAtWar())
            {
                if (MBRandom.RandomFloat < 0.9f)
                {
                    ...
                    mobileParty.SetMoveGoToSettlement(mobileParty.CurrentSettlement, MobileParty.NavigationType.Default, mobileParty.CurrentSettlement.HasPort);
            ...
            float avgDist = Campaign.Current.GetAverageDistanceBetweenClosestTwoTownsWithNavigationType(MobileParty.NavigationType.Default);
            float num13 = avgDist * 0.4f;
            float num14 = (84f + avgDist * 1.5f) * 0.5f;
            float num15 = (424f + 7.57f * avgDist) * 0.5f;
            ...
            foreach (KeyValuePair<ValueTuple<float, int>, Settlement> keyValuePair2 in sortedList)
            {
                ...
                else
                {
                    ...
                    int num32 = 0;
                    int num33 = 0;
                    if (item < 1f && mobileParty.IsWageLimitExceeded())
                    {
                    ...
                    {
                        Vec2 v = value2.GatePosition.ToVec2() - mobileParty.Position.ToVec2();
                        v.Normalize();
        ...
        private void CalculateVisitHideoutScoresForBanditParty(MobileParty mobileParty, Settlement currentSettlement, PartyThinkParams p)
        {
            ...
            float avgDist2 = Campaign.Current.GetAverageDistanceBetweenClosestTwoTownsWithNavigationType(MobileParty.NavigationType.Default);
            float num5 = (424f + 7.57f * avgDist2) / 2f;
            ...
            foreach (Hideout hideout2 in allHideouts)
            {
                ...
                {
                    float num6 = Campaign.Current.Models.MapDistanceModel.GetDistance(mobileParty, settlement, false, MobileParty.NavigationType.Default, out _);
        ...
        private SortedList<ValueTuple<float, int>, Settlement> FindSettlementsToVisitWithDistances(MobileParty mobileParty)
        {
            ...
            if (mobileParty.LeaderHero != null && mobileParty.LeaderHero.MapFaction.IsKingdomFaction)
            {
                if (mobileParty.Army == null || mobileParty.Army.LeaderParty == mobileParty)
                {
                    LocatableSearchData<Settlement> locatableSearchData = Settlement.StartFindingLocatablesAroundPosition(mobileParty.Position.ToVec2(), 30f);
                    for (Settlement settlement = Settlement.FindNextLocatable(ref locatableSearchData); settlement != null; settlement = Settlement.FindNextLocatable(ref locatableSearchData))
                    {
                        ...
                        {
                            float distance = mapDistanceModel.GetDistance(mobileParty, settlement, false, MobileParty.NavigationType.Default, out _);
                            if (distance < 350f)
                            {
                ...
                using (List<Settlement>.Enumerator enumerator = mobileParty.MapFaction.Settlements.GetEnumerator())
                {
                    while (enumerator.MoveNext())
                    {
                        ...
                        {
                            float distance2 = mapDistanceModel.GetDistance(mobileParty, settlement2, false, MobileParty.NavigationType.Default, out _);
                            if (distance2 < 350f)
                            {
            ...
            LocatableSearchData<Settlement> locatableSearchData2 = Settlement.StartFindingLocatablesAroundPosition(mobileParty.Position.ToVec2(), 50f);
            for (Settlement settlement3 = Settlement.FindNextLocatable(ref locatableSearchData2); settlement3 != null; settlement3 = Settlement.FindNextLocatable(ref locatableSearchData2))
            {
                ...
                {
                    float distance3 = mapDistanceModel.GetDistance(mobileParty, settlement3, false, MobileParty.NavigationType.Default, out _);
                    if (distance3 < 350f)
                    {
        ...
        private void AddBehaviorTupleWithScore(PartyThinkParams p, Settlement settlement, float visitingNearbySettlementScore)
        {
            AIBehaviorData item = new AIBehaviorData(
                settlement,
